#!/usr/bin/env python3
"""Phase 4b - Admission latency under concurrent load, FAIL-CLOSED config.

Identical methodology to scripts/phase4_latency.py (same tiers, same
sampling, same webhook-histogram-based measurement) so the two are directly
comparable. Only difference: this runs against the scoped fail-closed
webhook config (failurePolicy: Fail, namespaceSelector limited to
pci-cde=true, see policies/gatekeeper/webhook_failclosed_scoped.json)
applied for Task A, instead of the default failurePolicy: Ignore that
Phase 4 measured.

See phase4_latency.py's docstring for the full rationale on why the
webhook's own Prometheus histogram is used instead of client-side timing.
"""
import csv
import json
import os
...
def main():
    ...
    out_json = RESULTS_DIR / "phase4b_latency_failclosed.json"
    with open(out_json, "w") as f:
        json.dump(results, f, indent=2)

    out_csv = RESULTS_DIR / "phase4b_latency_failclosed_summary.csv"
